...
/**
 * Given path expression (list of paths separated by semicolon) through an association to a field,
 * return the value of that field on the first associated record.  Alternatively, can return the first
 * of a related list (a single list field).
 */
public class FirstInCollectionValueFunction implements ValueFunction
{
	...
	public Object getValue(Adaptation adaptation)
	{
		...
		{
			list = attributePath != null
				? AdaptationUtil
					.evaluatePath(adaptation, adaptationPaths, attributePath, ignoreNull)
				: AdaptationUtil.evaluatePath(adaptation, adaptationPaths);
		}
		else
		{
			list = adaptation.getList(attributePath);
		}
		return CollectionUtils.getFirstOrNull(list);
	}
